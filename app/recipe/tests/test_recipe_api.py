...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_ingredient_on_update(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(user=self.user, name='Water')
        self.assertIn(new_ingredient, recipe.ingredients.all())

    def test_update_recipe_assign_ingredient(self):
        """Test assigning an existing ingredient when updating a recipe."""
        ingredient1 = Ingredient.objects.create(user=self.user, name='Water')
        recipe = create_recipe(user=self.user)
    ...
    def test_filter_by_tags(self):
        ...
        r1 = create_recipe(user=self.user, title='Thahi Vegetable Curry')
        r2 = create_recipe(user=self.user, title='Aubergine With Tahini')
        tag1 = Tag.objects.create(user=self.user, name='Vegane')
